...
<br>
```{r}
lm(data = aah_summary_navon, formula = age ~ handedness) |> 
  emmeans(~ handedness) |> 
  contrast("pairwise") |> 
...
<br>
```{r}
lm(data = aah_summary_navon, formula = education ~ handedness) |> 
  emmeans(~ handedness) |> 
  contrast("pairwise") |> 
  summary(infer = T, adj = "none") |>
  as_tibble() |> 
...
<br>
```{r}
sex_data <- aah_summary_navon |> filter(sex %in% c("Male", "Female")) |> 
